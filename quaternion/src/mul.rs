        ...
        std::ops::Mul<Output = ValueType> + std::ops::Mul<ValueType, Output = Vector<ValueType, 3>>,
{
    ...
    ///
    /// For example given 3 quaternions, `pitch`, `yaw`, `roll` their
    /// multiplication `q = pitch * yaw * roll`, represents a transformation
    /// that first applies the roll, then the yaw and finally the pitch,
    /// just as a similar expression for transformation matrices would.
