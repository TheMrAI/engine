...
/// # use graphic::transform::inv_scale;
/// # use graphic::identity_matrix;
/// let radians = PI/2.0;
/// let S = scale(1.0, 2.0, 3.0);
/// let S_inv = inv_scale(1.0, 2.0, 3.0);
///
...
/// ```
#[rustfmt::skip]
pub fn inv_scale(scale_x: f32, scale_y: f32, scale_z: f32) -> Matrix<f32, 4, 4> {
...
/// Affine, orthogonal.
///
/// [Vector] based wrapper for [inv_scale].
/// 
/// # Panics
/// 
/// If any scalar is zero, or very close to zero.
...
/// # use graphic::transform::inv_scale_v;
/// # use graphic::identity_matrix;
/// # use lina::v;
/// let v = v![1.0, 2.0, 3.0];
/// let S = scale_v(v);
/// let S_inv = inv_scale_v(v);
///
/// let identity = identity_matrix();
///
/// assert_eq!(S * S_inv, identity);
/// ```
#[rustfmt::skip]
pub fn inv_scale_v(s: Vector<f32, 3>) -> Matrix<f32, 4, 4> {
    inv_scale(s[0], s[1], s[2])
}
